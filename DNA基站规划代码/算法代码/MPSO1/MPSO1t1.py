...
def run_mpso_within_time(p, max_runtime_seconds, times):
    all_results = []
    ...
    for tim in range(times):
        ...
        r = np.zeros(max_runtime_seconds)
        r[0] = np.random.rand()
        ...
        while time.time() - start_time < max_runtime_seconds:
            ...
            if t < max_runtime_seconds - 1:
                r[t+1] = 4 * r[t] * (1 - r[t])
            w = r[t] * w_min + (w_max - w_min) * (elapsed_time / max_runtime_seconds)
...
if __name__ == "__main__":
    ...
    results = run_mpso_within_time(p, max_runtime_seconds=max_runtime_seconds, times=20)

    df = pd.DataFrame(results)
    df.to_excel("MPSO_t1.xlsx", index=False)
