...
def run_drpso_within_time(p, max_runtime_seconds, times):
    all_results = []
    ...
    for tim in range(times):
        ...
        while time.time() - start_time < max_runtime_seconds:
            # 动态调整参数
            w = w_max - (w_max - w_min) * (t / max_runtime_seconds)**2
            b1 = 2 * np.sin((1 - t / max_runtime_seconds) * 0.5 * np.pi)
            b2 = 2 * np.cos((1 - t / max_runtime_seconds) * 0.5 * np.pi)
            ...
            # 差分进化策略
            F0 = 0.5
            suanzi = np.exp(1 - (max_runtime_seconds / (max_runtime_seconds + 1 - t)))
            F = F0 * 2**suanzi
            for i in range(N):
...
if __name__ == "__main__":
    ...
    results = run_drpso_within_time(p, max_runtime_seconds=max_runtime_seconds, times=20)

    df = pd.DataFrame(results)
    df.to_excel("DRPSO_t1.xlsx", index=False)
