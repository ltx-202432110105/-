...
def run_saompso_within_time(p, max_runtime_seconds, times):
    all_results = []
    ...
    for tim in range(times):
        ...
        while time.time() - start_time < max_runtime_seconds:
            ...
            w = 0.3 * np.sin((2 * np.pi * t) / (max_runtime_seconds * 10)) + 0.6
            c1 = np.sin((3 * np.pi * t) / (max_runtime_seconds * 10)) + 1.5
            c2 = np.cos((3 * np.pi * t) / (max_runtime_seconds * 10)) + 1.5
            
            for i in range(N):
                r1 = np.random.rand()
                r2 = np.random.rand()
                
                T = np.exp(-t / (max_runtime_seconds * 10))
                k = 1
                DDF = 0.35 * (1 + (5 / 7) * ((np.exp(t / (max_runtime_seconds * 10)) - 1) ** k) / ((np.exp(1) - 1) ** k))
                M = DDF * T
...
if __name__ == "__main__":
    ...
    results = run_saompso_within_time(p, max_runtime_seconds=max_runtime_seconds, times=20)

    df = pd.DataFrame(results)
    df.to_excel("SAOMPSO_t1.xlsx", index=False)
