...
import numpy as np
import pandas as pd
# from tetromino_generator import generate_special_area #固定模型数据
from model import *
...
def run_ahdpso_multiple_times(p, times):
    ...
    for tim in range(times):
        ...
        # special_area = generate_special_area(size=p['size']) #固定场景使用
...
file_path = r"D:\桌面\M_PSO\iterations.txt" #调用参数数据
params = read_parameters(file_path)

if __name__ == "__main__":
    p = canshu()
    results = run_ahdpso_multiple_times(p, times=20)

    df = pd.DataFrame(results)
    df.to_excel("AHDPSO_results.xlsx", index=False)
