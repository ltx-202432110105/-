...
def run_ahdpso_within_time(p, max_runtime_seconds, times):
    all_results = []
    ...
    for tim in range(times):
        ...
        t = 0
        last_record_time = start_time  # 上次记录时间
        next_target_time = 100  # 下一个目标时间点
        
        while time.time() - start_time < max_runtime_seconds:
            w = params['w'][t % len(params['w'])]
            c1 = params['c1'][t % len(params['c1'])]
            c2 = params['c2'][t % len(params['c2'])]
            c3 = params['c3'][t % len(params['c3'])]

            encoded_positions = [[['A', 'C', 'G', 'T'][int(round(val)) % 4] for val in pos] for pos in x]
            hamming_distances = []
            ...
            if len(candidates) > 1:
                xworst_idx = max(candidates, key=lambda idx: fitness[idx])
            else:
...
if __name__ == "__main__":
    ...
    results = run_ahdpso_within_time(p, max_runtime_seconds=max_runtime_seconds, times=20)

    df = pd.DataFrame(results)
    df.to_excel("AHDPSO_t1.xlsx", index=False)
